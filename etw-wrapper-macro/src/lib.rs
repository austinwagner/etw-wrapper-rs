...
fn impl_gen_etw_wrapper(args: &WrapperArgs) -> anyhow::Result<TokenStream2> {
    ...
    let providers = manifest
        .providers
        .iter()
        .map(|p| gen_provider(p, overrides.get(p.symbol.as_str()).copied()))
...
fn gen_provider(p: &Provider, name_override: Option<&Ident>) -> anyhow::Result<TokenStream2> {
    ...
    for ev in &p.events {
        let specs = ev
            .params
            .iter()
            .map(|t| classify(&t.win_type))
            .collect::<Vec<_>>();
        let helper = backing_name(&specs);
        backing
            .entry(helper.clone())
            .or_insert_with(|| gen_backing(&helper, &specs));
        let (method_ident, method) = gen_event_method(ev, &specs, &helper, p)?;
    ...
    Ok(quote! {
        ...
        impl #struct_name {
            ...
            pub fn register() -> ::etw_wrapper::Result<Self> {
                let guid = ::etw_wrapper::GUID::from_u128(#guid);
                let ctx = ::etw_wrapper::EtwLogger::register(&guid)?;
                ::etw_wrapper::Result::Ok(Self { ctx })
            }
...
fn plan_event(ev: &Event, p: &Provider, specs: &[FieldSpec]) -> anyhow::Result<Vec<FieldPlan>> {
    let n = ev.params.len();
    ...
    for i in 0..n {
        ...
        let plan = match kinds[i] {
            ...
            Kind::Unicode => {
                ...
                let temp = quote! {
                    let #tmp: &[u16] = &::etw_wrapper::field::to_u16cstring(#id);
                };
                let doc_ref = id.to_string();
                FieldPlan {
            ...
            Kind::FixedLenUnicode(len) => {
                ...
                let temp = quote! {
                    let #tmp: &[u16] =
                        &::etw_wrapper::field::to_u16cstring_fixed_len(#id, #len);
                };
                let doc_ref = id.to_string();
                FieldPlan {
                    param: Some((id, quote!(&str))),
            ...
            Kind::Utf8Ansi => {
                ...
                // Borrowing the buffer extends its lifetime to the end of the method body
                let temp = quote! {
                    let #tmp: &[u8] = &::etw_wrapper::field::to_cstring(#id);
                };
                let doc_ref = id.to_string();
                FieldPlan {
                    param: Some((id, quote!(&str))),
            ...
            Kind::FixedLenUtf8Ansi(len) => {
                ...
                let temp = quote! {
                    let #tmp: &[u8] =
                        &::etw_wrapper::field::to_cstring_fixed_len(#id, #len);
                };
                let doc_ref = id.to_string();
                FieldPlan {
                    param: Some((id, quote!(&str))),
            ...
            Kind::DerivedLen { from } => {
                ...
                let temp = quote! {
                    let #id: #int_ty = ::etw_wrapper::field::checked_len(#blob.len())?;
                };
...
fn gen_event_method(
    ...
    let plans = plan_event(ev, p, specs)?;
    ...
    Ok((
        ...
        quote! {
            #doc
            pub fn #method(&self, #(#params),*) -> ::etw_wrapper::Result<()> {
                const DESC: ::etw_wrapper::EVENT_DESCRIPTOR =
                    ::etw_wrapper::EVENT_DESCRIPTOR {
                        Id: #id,
                        Version: #version,
                        Channel: #channel,
                        Level: #level,
...
fn gen_backing(name: &str, specs: &[FieldSpec]) -> TokenStream2 {
    ...
    for (i, spec) in specs.iter().enumerate() {
        ...
        match spec.class {
            FieldClass::Scalar => {
                descs.push(quote! { ::etw_wrapper::field::scalar(&#arg) });
            }
            FieldClass::Bool => {
                let tmp = format_ident!("__b{}", i);
                temps.push(quote! { let #tmp: i32 = #arg as i32; });
                descs.push(quote! { ::etw_wrapper::field::scalar(&#tmp) });
            }
            FieldClass::Str => {
                descs.push(quote! { ::etw_wrapper::field::str16(#arg) });
            }
            FieldClass::AnsiStr => {
                descs.push(quote! { ::etw_wrapper::field::str8(#arg) });
            }
            FieldClass::Bytes => {
                descs.push(quote! { ::etw_wrapper::field::bytes(#arg) });
            }
            FieldClass::Sid => {
                descs.push(quote! { ::etw_wrapper::field::sid(#arg) });
            }
        }
    }
    ...
    quote! {
        fn #name(
            &self,
            desc: &::etw_wrapper::EVENT_DESCRIPTOR,
            #(#params),*
        ) -> ::etw_wrapper::Result<()> {
            #(#temps)*
            let data: [::etw_wrapper::field::EventDataDescriptor; #n] = [ #(#descs),* ];
...
fn classify(w: &WinType) -> FieldSpec {
    ...
    match w {
        ...
        WinType::SystemTime => scalar("y", quote!(::etw_wrapper::SYSTEMTIME)),
        WinType::Sid => FieldSpec {
            frag: "k",
            rust_ty: quote!(&::etw_wrapper::field::Sid),
...
mod tests {
    ...
    fn event_method_carries_message_doc() {
        ...
        let specs: Vec<_> = e.params.iter().map(|t| classify(&t.win_type)).collect();
        let p = Provider {
            symbol: "P".into(),
            guid: 0,
            events: vec![],
        };
        let (_, ts) = gen_event_method(&e, &specs, "__write_q", &p).unwrap();
