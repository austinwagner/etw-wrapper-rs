...
impl<'a> EventDataDescriptor<'a> {
    pub fn new(ptr: u64, size: u32) -> Self {
        Self {
            inner: EVENT_DATA_DESCRIPTOR {
                Ptr: ptr,
                Size: size,
...
pub fn scalar<T: Copy>(v: &T) -> EventDataDescriptor<'_> {
    EventDataDescriptor::new(v as *const T as u64, size_of::<T>() as u32)
}
...
pub fn bytes(b: &[u8]) -> EventDataDescriptor<'_> {
    EventDataDescriptor::new(b.as_ptr() as u64, b.len() as u32)
}
...
pub fn str16(buf: &[u16]) -> EventDataDescriptor<'_> {
    ...
    EventDataDescriptor::new(buf.as_ptr() as u64, size_of_val(buf) as u32)
}
...
pub fn sid(sid: &Sid) -> EventDataDescriptor<'_> {
    ...
    EventDataDescriptor::new(bytes.as_ptr() as u64, bytes.len() as u32)
}
